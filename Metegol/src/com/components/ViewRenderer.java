 ...
 * </p>
 *
 * @param <Param>  the type of parameters that define renders
 * @param <Render> the type of renders that are applied to views
 * @author Tom Gibara
 */
...
public abstract class ViewRenderer<Param, Render> {
     ...
     * count will cause the {@link ViewRenderer} to use a single thread that can
     * be shared among other instances.
     *
     * @param maxThreadCount        the number of threads that will be created to support
     *                              background rendering, may be zero.
     * @param passes                the number of rendering passes required to complete the
     *                              rendering required for a view
     * @param mayInterruptIfRunning whether background rendering tasks can be interrupted if they
     *                              become redundant
     * @param cacheCapacity         the maximum number of render
     */

    public ViewRenderer(int threadCount, int passes,
                        boolean mayInterruptIfRunning, int cacheCapacity) {
        if (threadCount < 0)
        ...
        mExecutor = threadCount == 0 ? getDefaultExecutor() : new Executor(
                threadCount);
        mPasses = passes;
     ...
     *
     * @param view  the view that will display the rendering
     * @param param the parameters that define the rendering
     */
    ...
    public void renderView(View view, Param param) {
        ...
        // see if there's a tag which will tell us about past rendering on this
        // view
        ...
        if (task == null)
            task = new Task(param);
        // associate the task with the view (and schedule it for rendering as
        // necessary)
     ...
     * called.
     *
     * @param timeout the number of milliseconds for which to wait for rendering
    ...
    public void stop() {
        try {
            ...
            throw new IllegalStateException(
                    "Impossible: interrupted without waiting");
        }
    }
     ...
     * </p>
     *
     * @param view              a view will be displayed to the user presently
     * @param param             defines the content that the view will display
     * @param immediatePassHint the pass to which rendering has already progressed
     */

    protected abstract void prepare(View view, Param param,
                                    int immediatePassHint);
     ...
     *
     * @param param defines the rendering that needs to be produced
     * @param pass  which rendering pass is being performed
     ...
     *
     * @param view   a view that needs to update with rendered content
     * @param render the content that is to be applied to the view
     * @param pass   the rendering pass that generated the rendered content
     */
     ...
     *
     * @param view a rendered view
     * @param tag  the object with which the view is to be tagged, may be null
     */
    ...
    private Task getTask(View view) {
        ...
        return obj == null || (obj instanceof ViewRenderer.Task) ? (Task) obj
                : null;
    }
    ...
    // ThreadPoolExecutor is broken for priority queues (see 6539720) so we need
    // to patch it.
    // Also newTaskFor() was only introduced in API level 9, so we can't use
    // that either
    ...
    private static class Executor extends ThreadPoolExecutor {
        ...
        public Executor(int threadCount) {
            // ThreadPoolExecutor can't be configured to grow the thread count
            // without risking queue rejections
            // so we take the easy way out and clamp it with a fixed number of
            // threads.
            // TODO introduce our own executor (or equivalent) that doesn't
            // share this weakness
            // TODO our own blocking queue which stitched-together lists of the
            // same priority would be much more efficient
        ...
        @SuppressWarnings("unchecked")
        private static class ComparableFuture<T> extends FutureTask<T>
                implements Comparable<ComparableFuture<T>> {
    ...
    private class Task implements Runnable, Callable<Void>, Comparable<Task> {
        ...
        private final Param mParam;        // immutable fields for task
        ...
        private int mPass = 0; // from the UI thread, this is actually 'the next
        // pass'
        ...
        public int compareTo(Task that) {
            if (this == that)
                return 0;
            if (this.mPass != that.mPass)
                return this.mPass < that.mPass ? -1 : 1;
            if (this.mOrder != that.mOrder)
                return this.mOrder < that.mOrder ? -1 : 1;
            return 0;
        }
